...
```
{
  "9091": "$PRESHARED_KEY", // Preshared Key
...
```
coap-client -m get -u "$USERNAME" -k "$PRESHARED_KEY" "coaps://$GATEWAYIP:5684/15004"
```
...
```
coap-client -m get -u "$USERNAME" -k "$PRESHARED_KEY" "coaps://$GATEWAYIP:5684/15001/$GROUPID"
```

> `$GROUPID` is an id from the list generated by the previous command. id's are 6 digits long.

## Get a list of all scenes
To get a complete list of all scenes use the following command:
```
coap-client -m get -u "$USERNAME" -k "$PRESHARED_KEY" "coaps://$GATEWAYIP:5684/15005/$GROUPID"
```

> `$GROUPID` is an id from the list generated by the `Get a list of all groups` command. id's are 6 digits long.
...
```
coap-client -m get -u "$USERNAME" -k "$PRESHARED_KEY" "coaps://$GATEWAYIP:5684/15001/$GROUPID/$SCENEID"
```

> `$SCENEID` is an id from the list generated by the previous command. id's are 6 digits long.
...
```
coap-client -m put -u "$USERNAME" -k "$PRESHARED_KEY" -e '{ "3312": [{ "5850": 1 }] }' "coaps://$GATEWAYIP:5684/15001/$DEVICEID"
```
